...
def meta_train(model_wrapper, meta_optimizer, train_loader, val_loader, inner_lr=0.01, inner_steps=1, meta_steps=1000):
    ...
    for step in range(meta_steps):
        ...
        for batch in train_loader:
            combined_ids = batch['combined_ids']
            answer_ids = batch['answer_ids']
            combined_ids, answer_ids = combined_ids.to(model_wrapper.device), answer_ids.to(model_wrapper.device)
            
            # Inner loop
            original_state = model_wrapper.model.state_dict()
            inner_optimizer = torch.optim.SGD(model_wrapper.model.parameters(), lr=inner_lr)
            for _ in range(inner_steps):
                inner_optimizer.zero_grad()
                outputs = model_wrapper.model(combined_ids, combined_ids)
                loss = criterion(outputs.view(-1, model_wrapper.vocab_size), answer_ids.view(-1))
                loss.backward()
                inner_optimizer.step()
            
            # Outer loop
            adapted_state = model_wrapper.model.state_dict()
            model_wrapper.model.load_state_dict(original_state)
            for param, adapted_param in zip(model_wrapper.model.parameters(), adapted_state.values()):
                param.grad = param.data - adapted_param
            meta_optimizer.step()
            
        if step % 10 == 0:
            val_accuracy = evaluate(model_wrapper, val_loader)
            print(f"Meta step {step}: Validation Accuracy = {val_accuracy}")
...
def main():
    ...
    meta_optimizer = optim.Adam(model_wrapper.model.parameters(), lr=1e-3)
    
    start_epoch = model_wrapper.load(MODEL_PATH, optimizer)

    meta_train(model_wrapper, meta_optimizer, train_loader, val_loader)

    model_wrapper.save(INCREMENTAL_MODEL_PATH, optimizer, start_epoch + 1)
