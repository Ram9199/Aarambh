...
def update_metadata(model_path, epoch, val_accuracy, val_f1):
    metadata = {
    ...
    if not os.path.exists(METADATA_FILE):
        with open(METADATA_FILE, 'w') as f:
            json.dump({'models': [metadata]}, f, indent=4)
    else:
...
def main():
    ...
    while True:
        ...
        for file_path in new_files:
            ...
            start_epoch += 3
            versioned_model_path = os.path.join(MODEL_DIR, f'aarambh_model_v{start_epoch}.pth')
            model_wrapper.save(versioned_model_path, optimizer, start_epoch)

            # Evaluate model and update metadata
            val_accuracy, val_f1 = evaluate(model_wrapper, val_loader)
            update_metadata(versioned_model_path, start_epoch, val_accuracy, val_f1)
