...
def train(model_wrapper, dataloader, epochs=3, lr=5e-5):
    ...
    for epoch in range(epochs):
        ...
        for batch in dataloader:
            ...
            # Debug print statements for tensor shapes
            print(f"src_ids_padded shape: {src_ids_padded.shape}")
            print(f"tgt_ids_padded shape: {tgt_ids_padded.shape}")

            optimizer.zero_grad()
            outputs = model(src_ids_padded, tgt_ids_padded)
            loss = loss_fn(outputs.view(-1, model_wrapper.vocab_size), tgt_ids_padded.view(-1))
            loss.backward()
            optimizer.step()
